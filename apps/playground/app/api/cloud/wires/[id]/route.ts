...
export async function PATCH(req: NextRequest, context: RouteContext): Promise<Response> {
  ...
  try {
    ...
    try {
      ...
      if (!(err instanceof WireNotFoundError) || payload.createIfMissing !== true) throw err;
...
export async function DELETE(req: NextRequest, context: RouteContext): Promise<Response> {
  const user = await requireApiKeyUser(req, ["wires:delete"]);
  if (user instanceof Response) return user;
...
function parseSource(value: unknown): WireSaveSource {
  if (value === "manual" || value === "json" || value === "reset" || value === "rename" || value === "chat") return value;
  return "manual";
}
