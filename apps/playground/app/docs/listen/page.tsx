        ...
        <table className="w-full min-w-[720px] border-collapse text-left text-[13px]">
          <thead className="bg-slate-50 text-[11px] uppercase tracking-wider text-slate-500 dark:bg-slate-800 dark:text-slate-400">
            <tr className="border-b border-slate-200 dark:border-slate-700">
              ...
              <th className="px-4 py-2.5 font-bold">Emitted by</th>
      ...
      <WireProvider defaultDiagram={DEMO_DIAGRAM} onEvent={handleEvent}>
        <div className="not-prose grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
          <div className="grid content-start gap-4">
            <section className="overflow-hidden rounded-lg border border-wire bg-wire-surface">
              <header className="flex items-center justify-between gap-2 border-b border-wire px-3.5 py-3">
            ...
            <section className="overflow-hidden rounded-lg border border-wire bg-wire-surface">
              <header className="grid gap-3 border-b border-wire px-3.5 py-3">
          ...
          <section className="overflow-hidden rounded-lg border border-wire bg-wire-surface">
            <header className="flex items-center justify-between gap-2 border-b border-wire px-3.5 py-3">
              ...
              <div className="flex items-center gap-2">
                <span className="rounded bg-wire-sunken px-1.5 py-0.5 font-mono text-[11px] text-wire-secondary wire-tabular">
                  {events.length}
                </span>
                {events.length > 0 ? (
                  <button
                    type="button"
                    onClick={() => setEvents([])}
                    className="text-[11px] font-bold uppercase tracking-wider text-wire-tertiary hover:text-wire-secondary"
                  >
                    Clear
                  </button>
                ) : null}
