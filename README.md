...
This repo includes a project skill at
[`.claude/skills/wire-mcp/SKILL.md`](.claude/skills/wire-mcp/SKILL.md). When
Claude Code is opened with this repo as the working directory, the skill is
available automatically. Invoke it directly with `/wire-mcp`, or ask naturally
for Wire diagrams, workflow diagrams, MCP flow diagrams, edge handles, React
editor usage, or diagram validation.

To use the same skill in another project:

```bash
mkdir -p .claude/skills
cp -R /path/to/wire/.claude/skills/wire-mcp .claude/skills/
```

To make it available across all Claude Code projects:

```bash
mkdir -p ~/.claude/skills
cp -R .claude/skills/wire-mcp ~/.claude/skills/
```

Claude Code discovers project skills from `.claude/skills/<skill-name>/SKILL.md`.
If a running Claude Code session started before the top-level `.claude/skills`
directory existed, restart that session so the directory is watched. After that,
edits to existing skills are picked up live.

The skill is instructions only. Configure the Wire MCP server separately as
`wire`; then use the actual tool prefix exposed by the host, such as
`wire__create_diagram`, `mcp__wire__create_diagram`, or the bare
`create_diagram` method depending on your client. When using the Claude Agent SDK, include `Skill` in
`allowed_tools` and load the `project` setting source so project skills are
discoverable.
